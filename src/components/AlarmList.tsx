...
import { Plus, Bell, Volume2, Trash2, Edit2, Play, Mic, Shield, Brain, Fingerprint, Zap } from 'lucide-react';
...
export const AlarmList: React.FC<AlarmListProps> = ({
  ...
  const handleTestTTS = (alarm: Alarm) => {
    ...
    const spoken = alarm.voiceGreeting.replace(/\[Name\]/gi, alarm.userName || 'Alex');
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className={`text-xl font-bold font-mono tracking-wide ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
            ALARM PROTOCOL REGISTRY
          </h2>
          <p className={`text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            100% Offline Personalized Vocal Alarms
          </p>
        </div>
      ...
      {alarms.length === 0 ? (
        <div className="p-12 text-center rounded-3xl border border-dashed border-slate-800 bg-slate-950/40 space-y-4">
          ...
          <div className="text-slate-300 font-mono text-sm">No alarm protocols registered.</div>
          <button
            onClick={onOpenCreateModal}
            ...
            return (
              <div
                ...
                className={`p-5 sm:p-6 rounded-3xl border transition-all ${
                  alarm.enabled
                    ? isDarkMode
                      ? 'border-cyan-500/30 bg-slate-950/80 shadow-lg shadow-cyan-950/30 text-white'
                      : 'border-cyan-300 bg-white shadow-md text-slate-900'
                    : isDarkMode
                    ? 'border-slate-800/80 bg-slate-950/30 opacity-60 text-slate-300'
                    : 'border-slate-200 bg-slate-50/80 opacity-60 text-slate-600'
                }`}
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-3">
                      {/* Big Digital Time (12-hour AM/PM by default) */}
                      <span className={`text-4xl sm:text-5xl font-black font-mono tracking-tight ${
                        isDarkMode 
                          ? 'text-white drop-shadow-[0_0_15px_rgba(0,240,255,0.3)]' 
                          : 'text-slate-900'
                      }`}>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                      ...
                      <span className="text-cyan-500 font-bold flex items-center space-x-1">
                        <Shield className="w-3 h-3" />
                        <span>Call: "{alarm.userName}"</span>
                      </span>
                    </div>
                  ...
                  <div className={`flex flex-col sm:items-end space-y-3 w-full sm:w-auto border-t sm:border-t-0 pt-3 sm:pt-0 ${
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-full border text-[10px] font-mono flex items-center space-x-1 ${
                        ...
                        <span className="capitalize">{alarm.soundTone.replace('_', ' ')}</span>
                      </span>
                      ...
                      <span className={`px-2.5 py-1 rounded-full border text-[10px] font-mono flex items-center space-x-1 ${
                        ...
                        <span className="capitalize">{alarm.challenge}</span>
                      </span>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-xl border font-mono text-xs flex items-center space-x-1 transition-all cursor-pointer ${
                          isDarkMode
                            ...
                            : 'bg-slate-100 hover:bg-slate-200 border-slate-200 text-cyan-800 font-bold'
                        }`}
                        title="Test Voice Name Greeting"
                      >
                      ...
                      <button
                        ...
                        title="Test Alarm Trigger Overlay"
                      >
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl border transition-colors cursor-pointer ${
                          isDarkMode
                            ...
                            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200 border-slate-200'
                        }`}
                        title="Edit Alarm"
                      >
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl border transition-colors cursor-pointer ${
                          isDarkMode
                            ? 'text-slate-500 hover:text-red-400 hover:bg-red-950/30 border-transparent'
