...
import { Clock, Bell, Volume2, Compass, Sun, Moon, Zap, Play } from 'lucide-react';
...
export const MainClock: React.FC<MainClockProps> = ({
  ...
  const formattedHours   = prefs.militaryTime ? String(hours).padStart(2, '0') : String(hours % 12 || 12).padStart(2, '0');
  ...
  if (activeAlarms.length > 0) {
    ...
    const sorted = activeAlarms
      .map(a => {
        ...
        if (alarmMinutes <= nowMinutes) alarmMinutes += 24 * 60;
  ...
  const handleTestGreeting = () => {
    ...
    const name = prefs.commanderName || 'Anshif';
    ttsService.speakText(`Good morning ${name}. Your alarm is set and ready.`, {
      pitch: 0.9,
      rate: 1.0,
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className={`relative rounded-3xl border p-8 sm:p-12 overflow-hidden shadow-2xl backdrop-blur-xl flex flex-col items-center justify-center text-center transition-colors duration-300 ${
        ...
        <div className="z-10 flex items-center space-x-3 mb-4 text-xs font-mono tracking-widest uppercase">
          <span className={`px-3 py-1 rounded-full border flex items-center space-x-1.5 font-bold ${
            isDarkMode ? 'bg-cyan-950/80 border-cyan-500/40 text-cyan-300' : 'bg-cyan-50 border-cyan-300 text-cyan-800'
          }`}>
          ...
          <span className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>EPOCH: {Math.floor(now.getTime() / 1000)}</span>
        </div>
        ...
        <div className="z-10 relative my-4 flex items-baseline justify-center font-mono select-none">
          <div className={`text-6xl sm:text-8xl md:text-9xl font-black tracking-tight ${
            isDarkMode ? 'text-white drop-shadow-[0_0_40px_rgba(0,240,255,0.5)]' : 'text-slate-900 drop-shadow-[0_4px_20px_rgba(0,180,216,0.25)]'
          }`}>
          ...
          <div className="ml-2 sm:ml-4 flex flex-col items-start space-y-1">
            ...
            {ampm && (
              <span className={`px-2.5 py-0.5 rounded-lg border text-xs sm:text-sm font-extrabold font-mono shadow-sm ${
                isDarkMode ? 'bg-cyan-950 border-cyan-500/50 text-cyan-300' : 'bg-cyan-100 border-cyan-300 text-cyan-900'
              }`}>
                {ampm}
              </span>
            )}
        ...
        <div className="z-10 mt-2 flex flex-wrap items-center justify-center gap-3">
          <div className={`px-4 py-1.5 rounded-full border text-xs font-mono flex items-center space-x-2 ${
            isDarkMode ? 'bg-slate-900/90 border-slate-800 text-slate-300' : 'bg-slate-100 border-slate-300 text-slate-700 shadow-sm'
          }`}>
            <span>👋 Wake name: <strong className={isDarkMode ? 'text-white' : 'text-slate-900'}>{prefs.commanderName || 'Anshif'}</strong></span>
          </div>
          <button
        ...
        <div className="z-10 mt-8 w-full max-w-md">
          {nextAlarm ? (
            <div className={`p-4 rounded-2xl border flex items-center justify-between text-left transition-all ${
              ...
              <div className="flex items-center space-x-3">
                <div className={`p-2.5 rounded-xl border ${
                  isDarkMode ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/40' : 'bg-cyan-200 text-cyan-800 border-cyan-300'
                }`}>
                  <Bell className="w-5 h-5 animate-bounce" />
                </div>
                <div>
                  <div className="text-[11px] font-mono font-bold text-cyan-600 dark:text-cyan-400 tracking-wider">NEXT ALARM</div>
                  <div className={`text-lg font-black font-mono ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    {formatTimeDisplay(nextAlarm.alarm.time, prefs.militaryTime)}{' '}
                    <span className={`text-xs font-normal ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>({nextAlarm.alarm.title})</span>
                  </div>
                  <div className={`text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                    In {nextAlarm.diffHours > 0 ? `${nextAlarm.diffHours}h ` : ''}{nextAlarm.diffMins}m
                  </div>
                </div>
              </div>
              <button
            ...
            <div className={`p-4 rounded-2xl border text-center ${isDarkMode ? 'bg-slate-900/60 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              <p className={`text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>No active alarms scheduled.</p>
              <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className={`p-5 rounded-2xl border flex items-center space-x-4 transition-all ${
          isDarkMode ? 'border-slate-800 bg-slate-950/70 text-white' : 'border-slate-200 bg-white/90 text-slate-900 shadow-sm'
        }`}>
          <div className={`p-3 rounded-xl border ${isDarkMode ? 'bg-cyan-950/40 text-cyan-400 border-cyan-500/30' : 'bg-cyan-100 text-cyan-800 border-cyan-200'}`}>
            <Clock className="w-6 h-6" />
          </div>
          <div>
            <div className={`text-2xl font-bold font-mono ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{activeAlarms.length}</div>
            <div className={`text-xs font-mono uppercase ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Active Alarms</div>
          </div>
        </div>
        ...
        <div className={`p-5 rounded-2xl border flex items-center space-x-4 transition-all ${
          ...
          <div className={`p-3 rounded-xl border ${isDarkMode ? 'bg-emerald-950/40 text-emerald-400 border-emerald-500/30' : 'bg-emerald-100 text-emerald-800 border-emerald-200'}`}>
            <Volume2 className="w-6 h-6" />
          </div>
          <div>
            <div className="text-sm font-bold font-mono text-emerald-600 dark:text-emerald-300">Voice Engine</div>
            <div className={`text-xs font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>Offline Speech Synthesis</div>
          </div>
        </div>
        ...
        <div className={`p-5 rounded-2xl border flex items-center space-x-4 transition-all ${
          ...
          <div className={`p-3 rounded-xl border ${isDarkMode ? 'bg-amber-950/40 text-amber-400 border-amber-500/30' : 'bg-amber-100 text-amber-800 border-amber-200'}`}>
